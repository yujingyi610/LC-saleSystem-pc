		...
		<el-dialog title="起诉" :visible.sync="Flag" width="900px" v-if="Flag">
			<el-card class="box-card">
				...
				<el-form :model="contractForm">
                    <el-row :gutter="20">
                        <el-col :span="12">
                            <el-form-item label="房号:">{{contractForm.hosueFullName}}</el-form-item>
                        </el-col>
                        <el-col :span="12">
...
<script>
import { prosecute } from '@/api/pages/mainPage/financialManagement/receivable'
export default {
  watch: {},
  data() {
    return {
  ...
  methods: {
    show(info) {
      ...
      this.contractForm.hosueFullName = '';
      ...
      if( info ){
            ...
            if( info.finBalanceForm ){
                this.contractForm.hosueFullName = info.finBalanceForm.houseFullName;// 房号
                
                this.balanceId = info.finBalanceForm.id;
            }else{
                this.contractForm.hosueFullName = info.houseFullName;
                this.contractForm.customName = info.customName;// 客户姓名
            ...
            if( info.resourceAttachmentList ){
                this.accessoryTable = info.resourceAttachmentList;
                this.accessoryTable.map(item=>{
    ...
    handleSave() {
      // 保存
      if( !this.indictForm.datOfFiling ){
          this.$message.error('请输入立案受理日期！')
          return
      }
      let resourceId = this.accessoryTable.map(item=>{
          return item.id
      })
      let data = {
          ...
          datOfFiling: this.indictForm.datOfFiling,
          prosecuteRemark: this.indictForm.prosecuteRemark,
          amountReceived: this.contractForm.receivedAmount,
          amountUnreceived: this.contractForm.unreceivedAmount,
          resourceId: resourceId,
          id: this.id
      }
      data = this.Qs.stringify(data)
      prosecute(data).then(res=>{
        if( res.status === 200 ){
