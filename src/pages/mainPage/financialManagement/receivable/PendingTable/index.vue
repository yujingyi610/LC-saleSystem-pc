<template>
    <div>
        ...
        <el-button type="primary" style="float: right; margin-bottom: 20px" @click="updateAll">批量更新</el-button>
         <el-table :data="tableData" border style="width: 100%">
            ...
            <el-table-column prop="unreceivedAmount" label="未收金额" min-width="80"></el-table-column>
            <!-- <el-table-column prop="houseDesc" label="签约日期" min-width="100"></el-table-column> -->
            ...
            <el-table-column prop="formCode" label="业务单号" min-width="120" show-overflow-tooltip ></el-table-column>
            <el-table-column label="操作" width="200"  >
                <template slot-scope="scope">
                    <div>
                        <el-button type="text" @click="handleReceivable(scope.row)">应收盘点</el-button>
                        <el-button type="text" @click="handleProsecute(scope.row)" v-if="!scope.row.finreceivableform || scope.row.finreceivableform.prosecutionState  != 1">起诉</el-button>
                        <el-button type="text" @click="handlePutOff(scope.row)" v-if="!scope.row.finreceivableform || scope.row.finreceivableform.dealOverdueStatus  != 1">延期</el-button>
                    </div>
                </template>
            </el-table-column>
        </el-table>
        ...
        <!-- 应收盘点弹框 -->
        <receivable-dialog ref="ReceivableDialogRef" @update="handleSearch"></receivable-dialog>
        <!-- 起诉弹框 -->
        <indict-dialog ref="indictDialogRef" @update="handleSearch"></indict-dialog>
        <!-- 延期 -->
        <put-off v-if="PutOffFlag" :row='row' @closeChild="handleCloseChild"></put-off>
        <!-- 批量更新弹框 -->
        <update-all-dialog ref="updateAllDialogRef" @update="handleSearch"></update-all-dialog>
        <!-- 查看签约单详情 -->
...
import PutOff from '@/components/form/PutOff'// 延期
import look from '../../../transactionManagement/contractManagement/submittedTable/look'// 点击房名查看签约单
import ReceivableDialog from './dialog/Receivable.vue'// 应收盘点弹框
import indictDialog from './dialog/indict.vue'// 起诉弹框
import updateAllDialog from './dialog/updateAll.vue'// 批量更新弹框
    export default {
        components: {
			longInput,
            houseDetail,
        ...
        methods: {
            ...
            handleSearch() {// 查询
                ...
                this.params = params;// 保存在全局变量中，批量更新时用
            ...
            updateAll() {// 打开批量更新弹框
                this.$refs.updateAllDialogRef.show(this.params)
            },
            handleSelectionChange(val) {// 保存复选框选中的table的数据
                this.multipleSelection = val;
            },
            handleLookSignInfo(row) {// 点击table某行触发，查看详细信息
                if( !row.trdSignForm ){
            ...
            handleReceivable(row) {// 应收处理
                ...
                getAccountReceivable(params).then(res=>{
                    console.log(res)
                    if( res.status === 200 ){
                        this.$refs.ReceivableDialogRef.show(res.data)
                    }
                })
                
            },
            handleProsecute(row) {// 起诉
                let params = new URLSearchParams();
                params.append('paymentId', row.paymentId)
                ...
                getAccountReceivable(params).then(res=>{
                    console.log(res)
                    if( res.status === 200 ){
                        this.$refs.indictDialogRef.show(res.data)
                    }
                })
            },
            handlePutOff(row) {// 
                ...
                getInfo({ id: row.trdSignForm.id }).then(res=>{
                    ...
                    if( res.status === 200 ){
                        if( !res.data.notChangeRemind ){
                            // let obj = {};// 查到的签约单详情里，付款信息的数目不止一条，需要匹配当前款项，并重新赋值
                            // for( let i = res.data.payments.length - 1; i >= 0; i-- ){
                            //     if( res.data.payments[i].paymentItemName === row.paymentItemName ){
                            //         obj = res.data.payments[i];
                            //     }
                            // }
                            this.row = res.data;
                            // this.row.payments = [obj];
