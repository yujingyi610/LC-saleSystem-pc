                    ...
                    <el-card>
                        ...
                        <el-card class="box-card">
                            ...
                            <div style="overflow: hidden">
                                <div style="float: left">
                                    <span>总价直减:</span>
                                    <el-input v-model="beautifyTotalPrice"></el-input>  
                                </div>
                                <div style="float: right; line-height: 40px">
        ...
        computed: {
            ...
            totalDiscount() {// 总折扣
                ...
                if( this.houseInfo ){
                    ...
                    return this.houseInfo.preTotalPrice - totalDiscountPrice;
                }else{
                    return 0
                }
        ...
        methods: {
            ...
            handleGetBank() {// 获取银行数据
                ...
                getBank(params).then(res=>{
                    ...
                    getHouse({ houseId: this.dataAll.NEW.houseId }).then(res=>{
                        this.DiscountInfo = [];
                        ...
                        if( res.status === 200 ){
                            if( this.redactFlag ){
                                ...
                                this.dataAll.NEW.discounts.map(item=>{
                            ...
                            this.dataAll.NEW.discounts.map(item1=>{
                            ...
                            // this.DiscountInfo = res.data.relDtos;
                            this.dataAll.NEW.discounts.map(i=>{
            ...
            handleData() {// 处理发送给后台的数据
                ...
                if( this.Payment === this.oldPayment ){
                    ...
                    for( let i = 0, len = payments.length; i < len; i ++ ){
                        console.log('i', i)
                        for( let j = 0, len2 = this.oldPayTable.length; j < len2; j ++ ){
                            console.log('j', j)
                            if( payments[i].paymentItemName.indexOf('房票') === -1 && this.oldPayTable[j].paymentItemName.indexOf('房票') === -1 ){
                                payments[i].beenDelayDays = this.oldPayTable[j].beenDelayDays;
                                break outloop;
                            }
                        }
                    }
            ...
            handleSave() {// 保存
                this.handleData();
                this.postData.payments.map(item=>{
                    if( !item.receivableDate ){
                        this.$message.error('请输入付款日期！');
                        return
                    }
                })
            ...
            show(dataAll) {
                ...
                if( dataAll ){
                    ...
                    this.table1.map(item=>{
                        item.date = item.date || this.signDate
                    })
                    this.table2.map(item=>{
                        item.date = item.date || this.signDate
                    })
                    ...
                    this.switchFlag3 = dataAll.NEW.houseTicketFlag?true:false;// 是否房票
                    this.ticketPrice = dataAll.NEW.houseTicketAmount;// 房票金额
                    ...
                    this.oldPriceType = data.discountCompMethod;
                    this.oldDiscountInfo = data.discounts;
                    this.oldDiscountInfo.map(item=>{
                        item.maxDiscountItem = item.discountItem
                    })
                    this.oldSwitchFlag3 = data.houseTicketFlag?true:false;
                    this.oldTicketPrice = data.houseTicketAmount;
                    this.oldBeautifyTotalPrice = data.beautifyTotalPrice;
                    this.oldTotalDiscount = data.discountAmount;
                    this.oldPrice = data.signPrice;
                    this.oldTotalprice = data.signTotalAmount;
                    this.oldIsBroken = data.underLowestFlag?'是':'否';
                    this.oldPayment = data.payMethod;
                    this.oldPayTable = data.payments;
