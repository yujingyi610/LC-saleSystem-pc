<template>
    <div>
        ...
        <el-table :data="tableData" border style="width: 100%" @row-click="handleLookRow">
            ...
            <el-table-column prop="title" label="流程标题" show-overflow-tooltip min-width="300"  ></el-table-column>
        ...
        <component :is="isShow" ref="componentRef" @closeChild="handleCloseChild"></component>
    </div>
</template>

<script>
import SignBackHouse from './backHouse'
import SignChangeName from './changeName'
import SignChangeHouse from './SignedChangeHouse'
import ContractChange from './ContractChange'
import PutOff from './PutOff'
import backHouse from './backHouse1'
import changeHouse from './changeHouse'
import changeName from './changeName1'
import PutOffSign from './PutOff1'
...
import {getList, deleteTrdChange, getInfo} from '@/api/pages/mainPage/transactionManagement/change/index.js'
    export default {
        watch: {
        ...
        methods: {
            ...
            handleRedact(row) {// 编辑
                ...
                getInfo({ id: row.id }).then(res=>{
                     ;
                    if( res.status === 200 ){
                        if( row.changeTypeText === "签约退房" ){
                            this.isShow = 'SignBackHouse';
                        }else if( row.changeTypeText === "签约更名" ){
                            this.isShow = 'SignChangeName';
                        }else if( row.changeTypeText === "签约换房" ){
                            this.isShow = 'SignChangeHouse';
                        }else if( row.changeTypeText === "签约变更" ){
                            this.isShow = 'ContractChange';
                        }else if( row.changeTypeText === "延期付款" ){
                            this.isShow = 'PutOff';
                        }else if( row.changeTypeText === "预定退房" ){
                            this.isShow = 'backHouse';
                        }else if( row.changeTypeText === "预定换房" ){
                            this.isShow = 'changeHouse';
                        }else if( row.changeTypeText === "预定更名" ){
                            this.isShow = 'changeName';
                        }else if( row.changeTypeText === "延期签约" ){
                            this.isShow = 'PutOffSign';
                        }else{
                            return
                        }
                        setTimeout(()=>{
                            this.$refs.componentRef.show(res.data)
                        },300)
                    }else{
            ...
            // 判断是因为，table中可以进行操作，弹框内的按钮也可以进行操作，row是table内的操作
                ...
                deleteTrdChange(params).then(res=>{
