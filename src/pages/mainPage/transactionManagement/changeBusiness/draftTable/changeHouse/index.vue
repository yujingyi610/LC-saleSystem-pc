                        ...
                        <el-card class="box-card">
                            ...
                            <div class="text item">
                                <el-table :data="paymentTable" border style="width: 100%; margin-bottom: 20px">
                                    ...
                                    <el-table-column prop="receivedAmount" label="已收金额（元）">
                                        <template slot-scope="scope">
                                            {{scope.row.receivedAmount || 0}}
                                        </template>
                                    </el-table-column>
                                    <el-table-column prop="unreceivedAmount" label="未收金额（元）">
                                        <template slot-scope="scope">
        ...
        methods: {
            ...
            handleConfirmHouse({houseInfo, buildInfo, batch}) {// 选择完房间后触发，关闭弹框
                ...
                getHouse({ houseId: houseInfo.id }).then(res=>{
                    ...
                    if( res.status === 200 ){
                        let data = res.data;
                        console.log(this.dataAll.OLD.projectCompanyName)
                        console.log(data.projectInfo.projectCompanyNames)
                        if( data.projectInfo.projectCompanyNames.indexOf(this.dataAll.OLD.projectCompanyName) !== -1 ){
            ...
            handleSave() {// 保存
                ...
                this.newPaymentTable.map(item=>{
                    data.payments.push({
                        ...
                        receivedAmount: item.receivedAmount,// 已收金额
                    })
                })
                data.discounts = null;
                data.tickets = null;
                data.houseId = this.newHouseInfo.houseId;
            ...
            show(dataAll) {
                if( dataAll ){
                    ...
                    this.newPaymentTable = dataAll.NEW.payments;
                    this.newPaymentTable[0].date = new Date().toLocaleString().split(' ')[0]
