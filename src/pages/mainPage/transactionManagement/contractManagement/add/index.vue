            ...
            <el-card class="box-card">
                <div slot="header" class="clearfix">
                    ...
                    <div style="float: right">
                        <el-button type="success" @click="handleSave" v-has="'trd:qy:add'" :disabled="canClick?false:''">保存</el-button>
                        <el-button type="success" @click="handleUp" v-has="'trd:qy:submit'" :disabled="canClick?false:''">提交审核</el-button>
                ...
                <div class="text item">
                        ...
                        <el-form :inline="true" :model="houseInfo">
                            <div>
                                ...
                                <el-form-item label="签约日期" style="float: right">
                                    <el-date-picker v-model="signDate" type="date" placeholder="选择日期" value-format="yyyy-MM-dd"></el-date-picker>
                                </el-form-item>
                            </div>
                            <div v-if="houseInfo">
                                <el-row>
                    ...
                    <el-card class="box-card">
                        ...
                        <el-table :data="BuyStamps" style="width: 100%" v-if="switchFlag2">
                            ...
                            <el-table-column label="优惠券号"></el-table-column>
                            <el-table-column label="使用金额（元）"></el-table-column>
                    ...
                    <el-card class="box-card">
                                ...
                                <el-table-column label="应收日期" width="180">
                                    <template slot-scope="scope">
                                        <span v-if="scope.row.name.indexOf('房票') !== -1">
                                            <el-date-picker v-model="scope.row.date" align="right" type="date" placeholder="选择日期" style="width: 140px" value-format="yyyy-MM-dd" disabled></el-date-picker>
                                        </span>
                                        <span v-else-if="scope.row.name === '一次性付款'">
                                            <el-date-picker v-model="scope.row.date" align="right" type="date" placeholder="选择日期" style="width: 140px" value-format="yyyy-MM-dd" @change="handleChangeTicketNum('table11')"></el-date-picker>
                                        </span>
                                        <span v-else>
                                            <el-date-picker v-model="scope.row.date" align="right" type="date" placeholder="选择日期" style="width: 140px" value-format="yyyy-MM-dd"></el-date-picker>
                                        </span>
                                    </template>
                                </el-table-column>
                            ...
                            <el-table-column label="应收日期" width="180">
                                <template slot-scope="scope">
                                    ...
                                    <span v-else-if="scope.row.name === '首付款'">
                                        <el-date-picker v-model="scope.row.date" align="right" type="date" placeholder="选择日期" style="width: 140px" value-format="yyyy-MM-dd" @change="handleChangeTicketNum('table21')"></el-date-picker>
                                    </span>
                                    <span v-else>
                                        <el-date-picker v-model="scope.row.date" align="right" type="date" placeholder="选择日期" style="width: 140px" value-format="yyyy-MM-dd"></el-date-picker>
                                    </span>
                        ...
                        <el-table v-else-if="Payment === '公积金贷款'" :data="table31" border style="width: 100%; margin-bottom: 20px">
                            ...
                            <el-table-column label="应收日期" width="180">
                                <template slot-scope="scope">
                                    <span v-if="scope.row.name.indexOf('房票') !== -1">
                                        <el-date-picker v-model="scope.row.date" align="right" type="date" placeholder="选择日期" style="width: 140px" disabled value-format="yyyy-MM-dd"></el-date-picker>
                                    </span>
                                    <span v-else-if="scope.row.name === '首付款'">
                                        <el-date-picker v-model="scope.row.date" align="right" type="date" placeholder="选择日期" style="width: 140px"  value-format="yyyy-MM-dd" @change="handleChangeTicketNum('table31')"></el-date-picker>
                                    </span>
                                    <span v-else>
                                        <el-date-picker v-model="scope.row.date" align="right" type="date" placeholder="选择日期" style="width: 140px" value-format="yyyy-MM-dd"></el-date-picker>
                                    </span>
                        ...
                        <el-table v-else-if="Payment === '组合贷款'" :data="table51" border style="width: 100%; margin-bottom: 20px">
                            ...
                            <el-table-column label="应收日期" width="180">
                                <template slot-scope="scope">
                                    ...
                                    <span v-else-if="scope.row.name === '首付款'">
                                        <el-date-picker v-model="scope.row.date" align="right" type="date" placeholder="选择日期" style="width: 140px" value-format="yyyy-MM-dd" @change="handleChangeTicketNum('table51')"></el-date-picker>
                                    </span>
                                    <span v-else>
                                        <el-date-picker v-model="scope.row.date" align="right" type="date" placeholder="选择日期" style="width: 140px" value-format="yyyy-MM-dd"></el-date-picker>
                                    </span>
                        ...
                        <el-table v-else-if="Payment === '分期付款'" :data="table41" border style="width: 100%; margin-bottom: 20px">
                            ...
                            <el-table-column label="应收日期" width="180">
                                <template slot-scope="scope">
                                    ...
                                    <span v-else>
                                        <el-date-picker v-model="scope.row.date" align="right" type="date" placeholder="选择日期" style="width: 140px" value-format="yyyy-MM-dd" @change="handleChangeTicketNum('table41')"></el-date-picker>
                                    </span>
                                </template>
                            </el-table-column>
                    ...
                    <el-card class="box-card">
                        ...
                        <el-form :inline="true">
                            ...
                            <el-form-item label="其他原因" v-if="!isRecord && why === '其他'">
...
} from "@/api/pages/mainPage/transactionManagement/contractManagement";
import {
  getCustomerInfo,
  getCustomerLabel
...
export default {
    ...
    totalprice() {
      ...
      if (this.houseInfo) {
        ...
        if (this.houseInfo.preTotalPrice - this.totalDiscount > 0) {
          price = Number(
            this.houseInfo.preTotalPrice - this.totalDiscount
          ).toFixed(0);
        } else {
          price = 0;
        }
        this.table11.map(item => {
          if (item.name === "一次性付款") {
            item.price =
              Number(
                price -
                  (this.switchFlag ? this.payCreditAmount : 0) -
                  (this.switchFlag3 ? this.ticketPrice : 0)
              ).toFixed(2) || 0;
          }
        });
    ...
    totalDiscount() {
      ...
      if (this.houseInfo) {
        ...
        this.multipleSelection.map(item => {
          // 遍历已选中的折扣方式，将三种折扣方式分别存储
          if (item.discountMethod === "比例优惠") {
            discountList.push(item);
          } else if (item.discountMethod === "单价优惠") {
            priceList.push(item);
          } else if (item.discountMethod === "总价优惠") {
            totalPricesList.push(item);
          }
        });
        if (this.priceType === "先单价后总价再折扣") {
          priceList.map(item => {
            totalDiscountPrice -= item.discountItem * this.houseInfo.priceArea;
          }); // 单价
          totalPricesList.map(item => {
            totalDiscountPrice -= item.discountItem;
          }); // 总价
          discountList.map(item => {
            totalDiscountPrice = totalDiscountPrice * item.discountItem * 0.01;
          }); // 折扣
  ...
  methods: {
    ...
    handleChangeTable() {
      ...
      if (!this.firstGoIn) {
        if (this.Payment === "一次性付款") {
          this.table11[this.table11.length - 1].price = Number(
            this.totalprice - this.payCreditAmount - this.ticketPrice
          ).toFixed(2);
        } else if (this.Payment === "商业贷款") {
          this.handleChangeNum2(
            this.table21[this.table21.length - 1],
            this.table21.length - 1
          );
        } else if (this.Payment === "公积金贷款") {
          this.handleChangeNum3(
            this.table31[this.table31.length - 1],
            this.table31.length - 1
          );
        } else if (this.Payment === "分期付款") {
          this.handleChangeNum4(
            this.table41[this.table41.length - 1],
            this.table41.length - 1
          );
        } else if (this.Payment === "组合贷款") {
          this.handleChangeNum5(
            this.table51[this.table51.length - 1],
            this.table51.length - 1
          );
        }
      }
      this.firstGoIn = false;
    },
    handlePrice(priceName) {
      // 工抵、房票改变，判断最大值
      this[priceName] = Number(this[priceName]);
      if (priceName === "ticketPrice") {
        if (
          this[priceName] >=
          this.totalprice - (this.switchFlag ? this.payCreditAmount : 0)
        ) {
          this[priceName] =
            this.totalprice - (this.switchFlag ? this.payCreditAmount : 0);
        }
        ...
        if (
          this[priceName] >=
          this.totalprice - (this.switchFlag ? this.ticketPrice : 0)
        ) {
          this[priceName] =
            this.totalprice - (this.switchFlag ? this.ticketPrice : 0);
        }
    ...
    handleChangeTicketNum(tableName) {
      // 改变首付款时间，房票时间动态改变
      let date = null;
        ...
        for (let i = 0; i < this[tableName].length; i++) {
          if (this[tableName][i].name === "一期款") {
            for (let j = 0; j < this[tableName].length; j++) {
              if (
                this[tableName][j].name ===
                this[tableName][i].name + "房票"
              ) {
    ...
    handleData() {
      ...
      if (this.Payment === "一次性付款") {
        payments = this.table11.map(item => {
          return {
            ...
            receivableAmount: isNaN(Number(item.price))
              ? 0
              : Number(item.price), // 应收款
      ...
      let data = {
        beautifyTotalPrice: Number(this.beautifyTotalPrice) || 0, // 美化金额
        ...
        batchId: this.batchInfo.releases, // 推盘批次ID
        ...
        noNetSignReason: this.why !== "其他" ? this.why : this.whyOther, // 未网签原因
        payCreditAmount: isNaN(Number(this.payCreditAmount))
          ? 0
          : Number(this.payCreditAmount), // 工抵金额
        // payCreditCompanyId: this.company.id,// 工抵城市公司
        payCreditFlag: this.switchFlag ? 1 : 0, // 是否工抵
        houseTicketFlag: this.switchFlag3 ? 1 : 0, // 是否房票
        houseTicketAmount: isNaN(Number(this.ticketPrice))
          ? 0
          : Number(this.ticketPrice), // 房票金额
    ...
    handleSave() {
      ...
      for (let i = 0; i < this.userTable.length; i++) {
        if (
          this.userTable[i].customName &&
          this.userTable[i].sex &&
          this.userTable[i].tel &&
          this.userTable[i].certificateName &&
          this.userTable[i].certificateNo
        ) {
      ...
      for (let i = 0; i < this.postData.payments.length; i++) {
        if (
          this.postData.payments[i].paymentItemName === "首付款" ||
          this.postData.payments[i].paymentItemName === "一次性付款" ||
          this.postData.payments[i].paymentItemName === "一期款"
        ) {
          console.log(
      ...
      if( this.postData.payMethod === '组合贷款' && this.postData.payments[this.postData.payments.length-1].receivableAmount == 0 ){
        this.$message.error("公积金部分金额必须大于0!");
        return
      }
      // for( let i = 0; i < this.postData.payments.length; i++){
    ...
    handleConfirmHouse({ houseInfo, buildInfo, batch }) {
      // 选择完房间后触发，关闭弹框
    ...
    handleDelect(index) {
      ...
      if (this.userTable.length >= 1) {
        ...
        if (index !== 0) {
          // 被删除的用户若不是第一个用户，则跳过以下步骤
          return;
        }
        this.counselorList[0].propertyConsultName = "";
        this.counselorList[0].propertyConsultTel = "";
        this.counselorList[0].propertyConsultId = "";
        if( !this.data || !this.data.subscribeId ){// 判断是否认购转签约，认购转签约商机不改变
          this.nicheId = '';
        }
        if (!this.userTable[0]) {
          return;
        ...
        let projectId = null;
        if (this.batchInfo.projectName) {
          projectId = this.batchInfo.projectName;
        } else if (this.data) {
        ...
        getCustomerInfo(params).then(r => {
          ...
          if (r.status === 200) {
            ...
            if (res != null && res != "") {
              ...
              this.counselorList[0].propertyConsultName =
                res.propertyConsultName;
              this.counselorList[0].propertyConsultTel =
                res.propertyConsultMobile;
    ...
    addUser() {
      ...
      this.$refs.findCustomRef.show(projectId);
    },
    handleChooseCustom(info) {
      // 得到客户信息，关闭弹框
      if (info) {
        console.log(info);
        for (let i = 0; i < this.userTable.length; i++) {
    ...
    handleChangeNum2(row, index, flag) {
      // 改变一个价格，另一个价格随之改变(商业贷款)
      if (flag) {
        if (row.name === "首付款") {
          this.table21[index + 1].price = Number(
            this.totalprice -
              this.payCreditAmount -
              this.ticketPrice -
              row.price
          ).toFixed(2);
        ...
        this.table21[index].price = Number(this.table21[index].price).toFixed(
          2
        );
      }
    },
    handleChangeNum3(row, index, flag) {
      // 改变一个价格，另一个价格随之改变(公积金贷款)
      if (flag) {
        if (row.name === "首付款") {
          this.table31[index + 1].price = Number(
            this.totalprice -
              this.payCreditAmount -
              this.ticketPrice -
              row.price
        ...
        this.table31[index].price = Number(this.table31[index].price).toFixed(
          2
        );
      }
    },
    handleChangeNum4(row, index, flag) {
      // 改变一个价格，另一个价格随之改变(分期贷款)
      if (flag) {
        let a = this.switchFlag3 ? 2 : 1;
        if (row.name === "一期款") {
          this.table41[index + a].price = Number(
            this.totalprice -
              this.payCreditAmount -
              this.ticketPrice -
              row.price -
              this.table41[index + 2 * a].price -
              this.table41[index + 3 * a].price -
              this.table41[index + 4 * a].price
          ).toFixed(2);
          if (this.table41[index + a].price < 0) {
            this.table41[index + a].price = 0;
            this.table41[index].price = Number(
              this.totalprice -
                this.payCreditAmount -
                this.ticketPrice -
                this.table41[index + 2 * a].price -
          ...
          this.table41[index - 4 * a].price = Number(
            this.totalprice -
              this.payCreditAmount -
              this.ticketPrice -
              row.price -
        ...
        this.table41[index].price = Number(this.table41[index].price).toFixed(
          2
        );
      }
    },
    handleChangeNum5(row, index, flag) {
      // 改变一个价格，另一个价格随之改变(组合贷款)
      if (flag) {
        let a = 1;
        if (row.name === "首付款") {
          this.table51[index + a].price = Number(
            this.totalprice -
              this.payCreditAmount -
              this.ticketPrice -
              row.price -
              this.table51[index + 2 * a].price
          ).toFixed(2);
          if (this.table51[index + a].price < 0) {
            this.table51[index + a].price = 0;
            this.table51[index].price = Number(
              this.totalprice -
                this.payCreditAmount -
                this.ticketPrice -
                this.table51[index + 2 * a].price
          ...
          this.table51[index - 2 * a].price = Number(
            this.totalprice -
              this.payCreditAmount -
              this.ticketPrice -
              row.price -
        ...
        this.table51[index].price = Number(this.table51[index].price).toFixed(
          2
        );
      }
    },
      ...
      getBank(params)
        .then(res => {
          ...
          if (res.status === 200) {
            ...
            console.log(this.bankOption1);
            console.log(this.bankOption2);
          }
        })
        .then(_ => {
          if (this.data) {
    ...
    handleGetProjectCompanyList(params) {
      //  获取项目公司下拉数据
    ...
    handleDelectStamps(index) {
      // 删除购房券
      this.BuyStamps.splice(index, 1);
    },
    handleSearchCounselor(row, index) {
      // 通过手机号搜索置业顾问
      // alert('通过手机号搜索置业顾问')
      let params = new URLSearchParams();
    ...
    handleSearchReferrer(row) {
      // 搜索推荐人
      this.searchPeopleFlag = true;
    },
    handleAddReferrer() {
      console.log(this.$refs.searchPeopleRef.currentRow);
    ...
    handleInfo() {
      ...
      this.commonResources = this.data.commonResources || []; // 传给后台的附件

      this.beautifyTotalPrice = this.data.beautifyTotalPrice; // 美化金额
      // this.counselorList = this.data.belongs;// 业绩归属
      this.data.belongs.map((item, index) => {
        this.counselorList[index] = item;
      });
      ...
      this.isRecord = this.data.netSignFlag ? true : false; // 是否网签/备案
      if (!this.isRecord) {
        if (
          this.data.noNetSignReason === "政府限网签" ||
          this.data.noNetSignReason === "无购房资格" ||
          this.data.noNetSignReason === "首付款不足"
        ) {
          this.why = this.data.noNetSignReason; // 未网签原因
        } else {
          this.why = "其他";
          this.whyOther = this.data.noNetSignReason;
        }
      }

      this.payCreditAmount = this.data.payCreditAmount; // 工抵金额
      // this.company.id = this.data.payCreditCityId;// 工抵项目公司
      this.company.company = this.data.payCreditCompanyName; // 工抵项目公司
      this.switchFlag = this.data.payCreditFlag ? true : false; // 是否工抵

      this.switchFlag3 = this.data.houseTicketFlag ? true : false; // 是否房票
      this.ticketPrice = this.data.houseTicketAmount; // 房票金额
      this.remark = this.data.remark; // 备注
      this.nicheId = this.data.nicheId; // 商机id
      ...
      console.log(this.data.devstagesId);
      let params = new URLSearchParams();
      params.append("devstagesId", this.data.devstagesId);
      getBank(params)
        .then(res => {
           ;
          if (res.status === 200) {
        ...
        .then(_ => {
          getHouse({ houseId: this.data.houseId }).then(res => {
            this.DiscountInfo = [];
            this.$refs.multipleTable.clearSelection();
            this.multipleSelection = [];
            if (res.status === 200) {
              if (this.redactFlag) {
                ...
                if (res.data.relDtos) {
                  // 编辑签约单时判断是否为公开优惠，置业顾问只可以选择公开优惠与经济总所选中的非公开优惠
              ...
              this.data.discounts.map(i => {
                this.$refs.multipleTable.toggleRowSelection(
                  this.DiscountInfo.find(d => d.discountId === i.discountId),
                  true
                );
              });
          ...
          let date = new Date();
          if (this.signDate) {
            this.signDate = new Date(this.signDate);
          } else {
            this.signDate = date;
          }
