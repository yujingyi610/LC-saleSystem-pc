<template>
    <div>
        ...
        <el-table :data="tableData" border style="width: 100%">
            ...
            <el-table-column label="操作"  >
                <template slot-scope="scope">
                    <!-- <el-button type="text" @click.stop.prevent="handleLook(scope.row)" v-has="'prj:price:view'">查看</el-button> -->
                    <el-button type="text" @click.stop.prevent="handleRedact(scope.row)" v-has="'prj:price:add'">编辑</el-button>
                    <el-button type="text" @click.stop.prevent="handleDelect(scope.row)" v-has="'prj:price:add'">删除</el-button>
                </template>
            </el-table-column>
        </el-table>
        <el-pagination
        ...
        <add-dialog ref="addDialogRef" @upDate="handlegetPriceSchemeList"></add-dialog>
    </div>
</template>
        ...
        methods: {
            ...
            handleRedact(row) {//编辑
                ...
                getPriceScheme(data).then(res=>{
                    ...
                    if( res.status === 200 ){
                        this.$refs.addDialogRef.show(res.data)
                    }
                })
            },
            handleDelect(row) {// 删除
                ...
                deletePriceScheme(data).then(res=>{
                     ;
                    if( res.status === 200 ){
                        this.$message({
                            type: 'success',
