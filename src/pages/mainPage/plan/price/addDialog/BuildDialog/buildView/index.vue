                        ...
                        <template slot-scope="scope">
                            <span v-if="scope.row[item.id]" v-for="(item2, index2) in scope.row[item.id]" :key="index2" class="cellItem">
                                <span class="cell_name">
                                    <el-checkbox v-model="item2.isChecked" @change="handlehouseCheck(item2.isChecked)" :disabled="((item2.houseStatus === 'FORSALE'|| item2.houseStatus === 'SALESCONTROL') && item2.lockType !== '1' && item2[valuation])?false:'disabled'"></el-checkbox>
						...
						if( key === property ){
                            ...
                            if( floorItem[key] ){
                                floorItem[key].map(houseItem=>{
                                    if( (houseItem.houseStatus === 'FORSALE' || houseItem.houseStatus === 'SALESCONTROL') && houseItem.lockType !== '1' && houseItem[this.valuation]){
                                        console.log(houseItem.isChecked);
                                        this.$set(houseItem, 'isChecked', flag);
                                        console.log(houseItem.isChecked);
                                        // houseItem.isChecked = flag
                                    }
