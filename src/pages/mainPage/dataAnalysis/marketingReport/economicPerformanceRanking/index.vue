<template>
  <div id="projectSalesDaily" class="projectDailySheet">
    <header class="head-title">经纪人业绩排名</header>
      ...
      <div>
        <el-form ref="searchForm" :model="searchForm" label-width="150px" label-position="right">
          ...
          <el-form-item label="排名方式">
            <el-radio-group v-model="searchForm.sortMethod" style="vertical-align:middle">
              <el-radio label="amount">按金额</el-radio>
              <el-radio label="count">按套数</el-radio>
            </el-radio-group>
          </el-form-item>
          ...
          <el-form-item label="统计日期" v-show="searchForm.sortTime == 'diy'">
            <el-date-picker suffix-icon="el-icon-date" type="date" :clearable="false" placeholder="开始日期" v-model="searchForm.rankStart"></el-date-picker><div style="width:auto;"> - </div>
            <el-date-picker suffix-icon="el-icon-date" type="date" :clearable="false" placeholder="结束日期" v-model="searchForm.rankEnd"></el-date-picker>
          </el-form-item>
        </el-form>
      </div>
      <div>
        ...
        <el-form label-width="120px" label-position="right">
          ...
          <el-form-item label="排名时段">
            <el-radio-group v-model="searchForm.sortTime" style="vertical-align:middle;width: 100%;">
              <el-radio label="day">今天</el-radio>
  ...
  export default {
    ...
    methods: {
      ...
      let CITY_ID = str;
        ...
        let ORDER_BY = typeof(this.searchForm.sortMethod)=='undefined'?'':this.searchForm.sortMethod;
        let DATE_SCOPE = typeof(this.searchForm.sortTime) == 'undefined'?'':this.searchForm.sortTime;
        //let DATE = typeof(this.searchForm.date)=='undefined'?'':y+'-'+M+'-'+d;
        let START_DATE = typeof(this.searchForm.rankStart) == 'undefined'?'':this.searchForm.rankStart;
        let END_DATE = typeof(this.searchForm.rankEnd) == 'undefined'?'':this.searchForm.rankEnd;
        ...
        this.tableSrc = CRM_GLOBAL.DATA_ACHIEVEMENT_RANK_TABLE+"&CITY_ID="+CITY_ID+"" +
          "&PROJECT_ID="+PROJECT_ID+"&ORDER_BYE="+ORDER_BY+"&IS_CHECKED="+IS_CHECKED+"&DATE_SCOPE="+DATE_SCOPE+"&START_DATE="+START_DATE+'&END_DATE='+END_DATE+'&REAL_NAME='+REAL_NAME+'&scroll=yes';
