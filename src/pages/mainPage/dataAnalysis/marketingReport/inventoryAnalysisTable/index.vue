<template>
	<div id="projectDailySheet" class="projectDailySheet">
    <header class="head-title">存量房源分析表</header>
      ...
      <div class="box">
        <el-form ref="searchForm" :model="searchForm" label-width="150px" label-position="right">
          ...
          <el-form-item label="统计方式">
            <el-radio-group v-model="searchForm.countMethod" style="vertical-align:middle">
              <el-radio label="productType">按产品类型</el-radio>
              <el-radio label="acreage">按面积段</el-radio>
              <el-radio label="area">按组团</el-radio>
              <el-radio label="termRoom">按现房期房对比</el-radio>
            </el-radio-group>
          </el-form-item>
          <el-form-item label="统计日期">
            <el-date-picker suffix-icon="el-icon-date" type="date" placeholder="请选择日期" :clearable="false" v-model="searchForm.date"></el-date-picker>
          </el-form-item>
        </el-form>
      </div>
      <div class="box">
        ...
        <el-form label-width="120px" label-position="right">
          ...
          <el-form-item label="统计范围">
            <el-select v-model="searchForm.scope" placeholder="请选择统计范围">
              <el-option
    ...
    <div id="iframeBox"
      ...
      <iframe id="innerRame" name="child" :src="tableSrc" frameborder="0"></iframe>
    </div>
	</div>
</template>
...
export default {
  ...
  methods: {
    ...
    statistics:function(){
      ...
      let CITY_ID = str;
      // let CITY_ID = typeof(this.searchForm.cityId)=='undefined'?'':this.searchForm.cityId==null?'':this.searchForm.cityId;
      let PROJECT_ID = typeof(this.searchForm.projectId)=='undefined'?'':this.searchForm.projectId==null?'':this.searchForm.projectId;
      let ESCROW_TYPE = typeof(this.searchForm.escrowType)=='undefined'?'':this.searchForm.escrowType==null?'':this.searchForm.escrowType;
      let IS_CHECKED = typeof(this.searchForm.isChecked)=='undefined'?'':this.searchForm.isChecked==null?'':this.searchForm.isChecked;
      let STATUS = typeof(this.searchForm.status)=='undefined'?'':this.searchForm.status==null?'':this.searchForm.status;
      let DATE = typeof(this.searchForm.date)=='undefined'?'':this.searchForm.date==null?'':y+'-'+M+'-'+d;
      let IS_MONEY = typeof(this.searchForm.isMoney)=='undefined'?'':this.searchForm.isMoney==null?'':this.searchForm.isMoney;
      let REAL_NAME = unescape(document.cookie.split("userRealName=")[1].split(";")[0]);
      let COUNT_METHOD = typeof(this.searchForm.countMethod)=='undefined'?'':this.searchForm.countMethod==null?'':this.searchForm.countMethod;
      let COUNT_SCOPE = typeof(this.searchForm.scope)=='undefined'?'':this.searchForm.scope==null?'':this.searchForm.scope;
      this.tableSrc = CRM_GLOBAL.DATA_STOCK_ANALYSIS+"&CITY_ID="+CITY_ID+"" +
        "&PROJECT_ID="+PROJECT_ID+"&ESCROW_TYPE="+ESCROW_TYPE+"&IS_CHECKED="+IS_CHECKED+"&STATUS="+STATUS+"&DATE="+DATE+"" +
        "&IS_MONEY="+IS_MONEY+'&REAL_NAME='+REAL_NAME+'&scroll=yes&COUNT_METHOD='+COUNT_METHOD+"&COUNT_SCOPE"+COUNT_SCOPE;
  ...
  created() {
    for(let i=1;i<25;i++){
      let obj = new Object();
      if(i<11){
        obj.label = '前'+this.numToWord[i-1]+'个月';
      }else if(i>10 && i<20){
        obj.label = '前十'+this.numToWord[i-11]+'个月';
      }else if(i==20){
        obj.label = '前二十个月';
      }else{
        obj.label = '前二十'+this.numToWord[i-21]+'个月';
      }
      obj.value = i;
  ...
  mounted:function(){
    ...
    let thisPage = document.getElementById('projectDailySheet');
