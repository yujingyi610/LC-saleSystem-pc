<template>
  <div id="debitDetail">
    <header class="head-title">应收款余额明细表</header>
      ...
      <div class="box">
        <el-form ref="searchForm" :model="searchForm" label-width="150px" label-position="right">
          ...
          <el-form-item label="结算日期">
            <el-date-picker suffix-icon="el-icon-date" type="date" placeholder="请选择日期" value-format="yyyy-MM-dd" v-model="searchForm.settle_start_date"></el-date-picker> -
            <el-date-picker suffix-icon="el-icon-date" type="date" placeholder="请选择日期" value-format="yyyy-MM-dd" v-model="searchForm.settle_end_date"></el-date-picker>
          </el-form-item>
        </el-form>
      </div>
      <div class="box">
        <el-form ref="searchForm" :model="searchForm" label-width="120px" label-position="right">
          ...
          <el-form-item label="付款方式">
            <el-select v-model="searchForm.payMethod" placeholder="请选择付款方式">
              <el-option
...
<script>
  ...
  import { getPayMethods } from "@/api/pages/mainPage/financialManagement/housingFund"
  export default {
    data(){
      return{
        name:'收款明细表1',
    ...
    methods:{
      ...
      statistics(){
        ...
        let pay_method = typeof(this.searchForm.pay_method)=='undefined'?'':this.searchForm.pay_method==null?'':this.searchForm.pay_method;
        let settle_start_date = typeof(this.searchForm.settle_start_time)=='undefined'?'':this.searchForm.settle_start_time==null?'':this.searchForm.settle_start_time;
        let settle_end_date = typeof(this.searchForm.settle_end_time)=='undefined'?'':this.searchForm.settle_end_time==null?'':this.searchForm.settle_end_time;

        this.tableSrc = CRM_GLOBAL.DATA_SHOULD_DEBIT_DETAIL+"&project_id="+project_id+"&contract_start_date="+contract_start_date+"&contract_end_date="+contract_end_date+"&" +
          "user_id="+user_id+"&pay_method="+pay_method+"&settle_start_date="+settle_start_date+"&settle_end_date="+settle_end_date;
