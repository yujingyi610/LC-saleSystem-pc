<template>
  <div id="debitDetail">
    <header class="head-title">收款明细表</header>
    <div class="head-container">
      ...
      <el-form ref="searchForm" :model="searchForm" label-width="150px" label-position="right">
        <el-form-item label="款项类型">
            <el-checkbox-group v-model="searchForm.payment_item_name">
              <el-checkbox label="认筹金"></el-checkbox>
    ...
    export default {
      data(){
          return{
            name:'收款明细表1',
            searchForm:{
              payment_item_name:['一次性付款','首付款','商业按揭款','公积金','分期款','工抵','房票']
            },
            tableSrc:null,
            projectOption:[],
            cityOption:[],
      ...
      methods:{
        ...
        statistics(){
          ...
          let payment_item_name = this.searchForm.payment_item_name.length==0?'':this.searchForm.payment_item_name.toString();

          this.tableSrc = CRM_GLOBAL.DATA_DEBIT_DETAIL+"&city_id=" + city_id +"&project_id="+project_id+"&receive_start_date="+receive_start_date+"&receive_end_date="+receive_end_date+"&user_id="+user_id+"&check_status="+check_status+"&payment_item_name="+payment_item_name;
