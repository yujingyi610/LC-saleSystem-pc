        ...
        <div v-show="!redactFlag">
            ...
            <el-card class="box-card">
                ...
                <el-table :data="payTable" border style="width: 100%; margin-bottom: 20px">
                    <el-table-column prop="paymentItemName" label="款项"></el-table-column>
                    ...
                    <el-table-column label="是否工行">
                        <template slot-scope="scope">
                            {{scope.row.icbcFlag?'是':'否'}}
                        </template>
                    </el-table-column>
                    <el-table-column prop="bankBranchName" label="银行/单位"></el-table-column>
...
<script>
import {saveAdd} from '@/api/pages/mainPage/transactionManagement/contractManagement'
        ...
        methods: {
            ...
            show(data, redactFlag) {
                ...
                if( data ){
                    ...
                    if(data.payments){
                        this.payTable = data.payments.map(item=>{
                            return {
                                type: item.type,
                                signId: item.signId,
                                ...
                                delayStartDate: (new Date()).toLocaleString(),// 延期开始日期
                                delayEndDate: '',// 延期结束日期
                                delayFlag: 1,// 是否延期 1：是 ；0：否
                                beenDelayDays: 0,// 已经延期天数(天)
                                delayDays: 0,// 延期天数(天)
                                totalDelayDays: 0,// 累计延期天数(天)
                                receivableDate: item.receivableDate,// 应收日期
                                interestFlag: 1,// 是否计息 1：是；0：否
                                interestRate: 0,// 利率（%）
                                ...
                                icbcFlag: item.icbcFlag,// 是否工商银行 1:是;0:否
                            }
                        })
                    }
                    if( !redactFlag ){
                    ...
                    this.accessoryTable = data.resourceMaps || [];// 展示附件
                    this.commonResources = data.commonResources || [];// 传给后台的附件
