        ...
        <div class="add-con">
            <el-card class="box-card">
                <div slot="header" class="clearfix">
                    <span class="title">签约更名审批单发起</span>
    ...
    export default {
        ...
        methods: {
            ...
            addUser() {// 点击新增客户按钮
				...
				this.$refs.findCustomRef.show(this.row.projectId)
            },
            ...
            handleSave() {// 保存
                for( let i = 0; i < this.newUserTable.length; i++ ){
                    ...
                    if ( (this.newUserTable[i].certificateName === "居民身份证" || this.newUserTable[i].certificateName === "临时身份证") && !this.idCard(this.newUserTable[i].certificateNo) ) {
                        this.$message.error("身份证格式错误！");
                        return;
                    }
                    if ( this.isName(this.newUserTable[i].customName) !== 1 ) {
                        this.$message.error(this.isName(this.newUserTable[i].customName));
                        return;
                    }
                    if ( this.sexFlag(this.newUserTable[i].certificateNo, this.newUserTable[i].certificateName, this.newUserTable[i].sex) !== 1 ) {
                        this.$message.error(this.sexFlag(this.newUserTable[i].certificateNo, this.newUserTable[i].certificateName, this.newUserTable[i].sex));
                        return;
                    }
                    if ( this.noHanzi(this.newUserTable[i].certificateNo, this.newUserTable[i].certificateName) !== 1 ) {
                        this.$message.error(this.noHanzi(this.newUserTable[i].certificateNo, this.newUserTable[i].certificateName));
                        return;
                    }
                    if ( this.otherCardNum(this.newUserTable[i].certificateNo) !== 1 ) {
                        this.$message.error(this.otherCardNum(this.newUserTable[i].certificateNo));
                        return;
                    }
                }
