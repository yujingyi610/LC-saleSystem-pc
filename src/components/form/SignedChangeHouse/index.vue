                    ...
                    <el-card class="box-card">
                        ...
                        <el-card class="box-card">
                            ...
                            <el-table :data="PayTable" border style="width: 100%; margin-bottom: 20px">
                                ...
                                <el-table-column prop="date" label="应收日期" width="180"></el-table-column>
                                <el-table-column prop="icbcFlag" label="是否工行"></el-table-column>
                                <el-table-column prop="bank" label="银行/单位"></el-table-column>
                    ...
                    <el-card class="box-card">
                        ...
                        <el-form :inline="true">
                            ...
                            <el-form-item label="房票金额" v-if="newSwitchFlag3">
                                <el-input v-model="newTicketPrice" @blur="handlePrice('newTicketPrice')"></el-input>
                            </el-form-item>
                        </el-form>
                        <!-- <div style="overflow: hidden">
                            <span class="title_text">是否有特殊折扣:
                        ...
                        <div style="overflow: hidden">
                            <div style="float: left">
                                <span>总价直减:</span>
                                <el-input v-model="newBeautifyTotalPrice" @change="handleBeautifyPrice"></el-input>  
                            </div>
                            <div style="float: right; line-height: 40px">
        ...
        computed: {
            newPrice() {// 签约单价
                if( this.newHouseInfo ){
                    if( this.newTotalprice && this.newHouseInfo.priceArea ){
            ...
            newTotalprice() {// 签约总价
                if( this.newHouseInfo ){
                    ...
                    if( this.newHouseInfo.preTotalPrice - this.newTotalDiscount > 0 ){
                        price = Number(this.newHouseInfo.preTotalPrice - this.newTotalDiscount).toFixed(0)
                    }else{
                        price = 0;
                    }
                    this.table11.map(item=>{
                        if( item.name === '一次性付款' ){
                            item.price = Number(price - this.payCreditAmount - this.ticketPrice).toFixed(2) || 0;
                        }
                    })
            ...
            newTotalDiscount() {// 总折扣
                ...
                if( this.newHouseInfo ){
                    ...
                    return (this.newHouseInfo.preTotalPrice - totalDiscountPrice).toFixed(2);
                }else{
                    return 0
                }
        ...
        methods: {
                ...
                getHouse({ houseId: houseInfo.id }).then(res=>{
                    ...
                    if( res.status === 200 ){
                        ...
                        if( data.projectInfo.projectCompanyNames.indexOf(this.row.projectCompanyName) !== -1 ){
                            ...
                            // this.newDiscountInfo = data.relDtos;
            ...
            handleChangeTable() {// 工抵、房票改变，付款table展示改变
                ...
                if( this.newPayment === '一次性付款' ){
                    this.table11[this.table11.length - 1].price = Number(this.newTotalprice - this.payCreditAmount - this.newTicketPrice).toFixed(2);
            ...
            handlePrice(priceName) {// 工抵、房票改变，判断最大值
                ...
                if( priceName === 'newTicketPrice' ){
                    if( Number(this[priceName]) >= Number(this.newTotalprice) ){
                        this[priceName] = this.newTotalprice;
                    }
                }
            ...
            handleChangeNum2(row, index) {// 改变一个价格，另一个价格随之改变(商业贷款)
                if( row.name === '首付款' ){
                    this.table21[index+1].price = Number(this.newTotalprice - this.payCreditAmount - this.newTicketPrice - row.price).toFixed(2);
            ...
            handleChangeNum3(row, index) {// 改变一个价格，另一个价格随之改变(公积金贷款)
                if( row.name === '首付款' ){
                    this.table31[index+1].price = Number(this.newTotalprice - this.payCreditAmount - this.newTicketPrice - row.price).toFixed(2);
                }else if( row.name === '公积金' ){
                    this.table31[index-1].price = Number(this.newTotalprice - this.payCreditAmount - this.newTicketPrice - row.price).toFixed(2);
                }
            ...
            handleChangeNum4(row, index) {// 改变一个价格，另一个价格随之改变(分期贷款)
                ...
                if( row.name === '一期款' ){
                    this.table41[index+a].price = Number(this.newTotalprice - this.payCreditAmount - this.newTicketPrice - row.price - this.table41[index+2*a].price - this.table41[index+3*a].price - this.table41[index+4*a].price).toFixed(2);
                    if( this.table41[index+a].price < 0 ){
                        this.table41[index+a].price = 0;
                        this.table41[index].price = Number(this.newTotalprice - this.payCreditAmount - this.newTicketPrice - this.table41[index+2*a].price - this.table41[index+3*a].price - this.table41[index+4*a].price).toFixed(2);
                    }
                }else if( row.name === '二期款' ){
                    this.table41[index+a].price = Number(this.newTotalprice - this.payCreditAmount - this.newTicketPrice - row.price - this.table41[index+2*a].price - this.table41[index+3*a].price - this.table41[index-a].price).toFixed(2);
                    if( this.table41[index+a].price < 0 ){
                        this.table41[index+a].price = 0;
                        this.table41[index].price = Number(this.newTotalprice - this.payCreditAmount - this.newTicketPrice - this.table41[index+2*a].price - this.table41[index+3*a].price - this.table41[index-a].price).toFixed(2);
                    }
                }
                else if( row.name === '三期款' ){
                    this.table41[index+a].price = Number(this.newTotalprice - this.payCreditAmount - this.newTicketPrice - row.price - this.table41[index+2*a].price - this.table41[index-a].price - this.table41[index-2*a].price).toFixed(2);
                    if( this.table41[index+a].price < 0 ){
                        this.table41[index+a].price = 0;
                        this.table41[index].price = Number(this.newTotalprice - this.payCreditAmount - this.newTicketPrice - this.table41[index+2*a].price - this.table41[index-a].price - this.table41[index-2*a].price).toFixed(2);
                    }
                }
                else if( row.name === '四期款' ){
                    this.table41[index+a].price = Number(this.newTotalprice - this.payCreditAmount - this.newTicketPrice - row.price - this.table41[index-3*a].price - this.table41[index-a].price - this.table41[index-2*a].price).toFixed(2);
                    if( this.table41[index+a].price < 0 ){
                        this.table41[index+a].price = 0;
                        this.table41[index].price = Number(this.newTotalprice - this.payCreditAmount - this.newTicketPrice - this.table41[index-3*a].price - this.table41[index-a].price - this.table41[index-2*a].price).toFixed(2);
                    }
                }
                else if( row.name === '五期款' ){
                    this.table41[index-4*a].price = Number(this.newTotalprice - this.payCreditAmount - this.newTicketPrice - row.price - this.table41[index-3*a].price - this.table41[index-a].price - this.table41[index-2*a].price).toFixed(2);
                    if( this.table41[index+a].price < 0 ){
                        this.table41[index+a].price = 0;
                        this.table41[index].price = Number(this.newTotalprice - this.payCreditAmount - this.newTicketPrice - this.table41[index+2*a].price - this.table41[index-a].price - this.table41[index-2*a].price).toFixed(2);
                    }
            ...
            handleChangeNum5(row, index) {// 改变一个价格，另一个价格随之改变(组合贷款)
                let a = 1;
                if( row.name === '首付款' ){
                    this.table51[index+a].price = Number(this.newTotalprice - this.payCreditAmount - this.newTicketPrice - row.price - this.table51[index+2*a].price).toFixed(2);
                    if( this.table51[index+a].price < 0 ){
                        this.table51[index+a].price = 0;
                        this.table51[index].price = Number(this.newTotalprice - this.payCreditAmount - this.newTicketPrice - this.table51[index+2*a].price).toFixed(2);
                    }
                }else if( row.name === '按揭款' ){
                    this.table51[index+a].price = Number(this.newTotalprice - this.payCreditAmount - this.newTicketPrice - row.price - this.table51[index-a].price).toFixed(2);
                    if( this.table51[index+a].price < 0 ){
                        this.table51[index+a].price = 0;
                        this.table51[index].price = Number(this.newTotalprice - this.payCreditAmount - this.newTicketPrice - this.table51[index-a].price).toFixed(2);
                    }
                }
                else if( row.name === '公积金' ){
                    this.table51[index-2*a].price = Number(this.newTotalprice - this.payCreditAmount - this.newTicketPrice - row.price - this.table51[index-a].price).toFixed(2);
            ...
            handleInfo() {// 处理编辑里面的数据
                ...
                this.DiscountInfo = this.row.discounts;
