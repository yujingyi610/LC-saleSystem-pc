...
import {saveTrdChange, submitTrdChange} from '@/api/pages/mainPage/transactionManagement/change/index.js'
    export default {
        ...
        computed: {
            ...
            totalSignDelayDays() {
                return this.signDelayDays + (this.row.totalDelayDays || 0)
            }
        },
        data() {
            return{
        ...
        methods: {
            ...
            handleSave() {// 保存
                ...
                data.signDate = this.Time(this.planSignDate);
                data.signDelayDate = this.signDelayDate;
                data.signDelayDays = this.signDelayDays;
                data.beenSignDelayDays = this.row.totalDelayDays;
            ...
            handleUp() {// 提交
                ...
                data.signDelayDate = this.signDelayDate;
                data.signDelayDays = this.signDelayDays;
                data.beenSignDelayDays = this.row.beenSignDelayDays;
                data.totalSignDelayDays = this.totalSignDelayDays;
                data.businessId = this.row.id;
                data.id = null;
                data.customs = null;
        ...
        created() {
            if( this.row ){
                this.code = this.row.subscribeCode;// 合同单号
                this.signDate = this.row.subscribeDate;// 签约日期
                this.houseInfo = this.row;// 房间信息
                this.userTable = this.row.customs;// 客户table
            
                this.remark = this.row.remark;// 备注

                this.table = [this.row.trdTradePayment];
                this.table[0].date = new Date().toLocaleString().split(' ')[0];
                this.planSignDate = this.row.realPlanSignDate;// 计划签约日期
                this.signDelayDate = new Date(this.row.realPlanSignDate);// 延期到期日期
                // this.signDelayDays = 0;// 延期天数
                // this.totalSignDelayDays = 0;// 累计延期天数
